...
chatApp.controller('LoginController', function($scope, focus){
	...
	$scope.createChatSession = function(id, isCallBack){
		...
		if(index===false){
			...
			if(index!==false){
				...
				focus('focusMe'+id);
				//$scope.chatSessionList[index]['ChatMessages'].focus();
	...
	$scope.socket.on('callBackLogin', function(data){
		if(data.UserID!=null){
			...
			$scope.addNotification($scope.userID, 'Welcome to the Chat App '+$scope.usersList[$scope.userID].firstName+ '! :)');
			$scope.$apply();
	...
	$scope.socket.on('callBackPrivateMessage',function(senderID, message){
		...
		$scope.notifyMe(senderID, $scope.usersList[senderID].firstName);
		$scope.$apply();
	});
	...
	$scope.notifyMe = function(senderID, senderName) {
	  if (!Notification) {
	  ...
	  var notification = new Notification('New Message', {
	    icon: '/chatIcon',
	    body: "Hey there! "+ senderName + " sent a message.",
	  });
