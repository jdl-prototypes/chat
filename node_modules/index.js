...
io.on('connection', function(socket){
  ...
  socket.on('login', function(data){
    ...
    if(!uID){
      count++;
      userData[count] = {'ID':count,'firstName':fName, 'lastName':lName,'initials':initials, 'dpColor':dpColor, 'SessionID':[socket.id]};
  ...
  function getIDBySessionID(sessionID){
    ...
    for(var user in userData){console.log(user.firstName);
      for(var sID in user.SessionID){console.log(sID+":"+user.SessionID);
        if(sID==sessionID){
          returnValue=user.ID;break;
        }
      }
    }
    return returnValue;
