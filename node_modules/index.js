...
io.on('connection', function(socket){
  ...
  socket.on('login', function(data){
    var userID = findUsername(data.firstName, data.lastName);
    if(userID===false){
      count++;
      userData[count] = {'firstName':data.firstName, 'lastName':data.lastName,'initials':data.initials, 'dpColor':data.color, 'SessionStatus':1};
      socket.emit('callBackLogin', {'UserID':count, 'firstName':data.firstName, 'lastName':data.lastName});
    }
  ...
  function findUsername(firstName, lastName){
    ...
    for(var i=1;i<=count;i++){
      var data = userData[i];
      returnValue = (data['firstName']==firstName && data['lastName']==lastName)?data['UserID']:false;
      break;
    }
    return returnValue;
  }
