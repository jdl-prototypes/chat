...
<html ng-app='chatApp'>
  ...
  <body ng-controller='LoginController'>
    ...
    <nav ng-show='showUserList'>
      ...
      <ul id=''>
        <li ng-repeat="user in usersList" ng-if="(user.firstName!=null || user.lastName!=null) && (user.Status==1) && (user.ID!=userID)" ng-click="createChatSession(user.ID)">
    ...
    <footer>
      ...
      <div ng-repeat="session in chatSessionList">
        ...
        <div class='pad5' ng-show="session.ChatSessionStatus">
          <div class="childDisplayInlineBlock widthFull displayRow pad5 lineBottom" ng-repeat="data in session.ChatMessages[session.ID]">
        ...
        <div ng-show="session.ChatSessionStatus"><input type='text' ng-model='session.Txt' ng-keyup="$event.keyCode == 13 ?sendPrivateMessage(session.ID,userID,session.Txt): null;"/></div>
      </div> 
