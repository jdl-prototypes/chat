  ...
  <body ng-controller='LoginController'>
    ...
    <footer>
      <div ng-repeat="session in chatSessionList">
        <div><div>{{session.firstName}}</div><button ng-click="deleteChatSession(session.ID)">X</button><button>_</button></div>
        <div></div>
      ...
      <div>
        <div><div></div><button ng-click="deleteChatSession(session.ID)">X</button><button>_</button></div>
        <div></div>
        <div><input type='text'/></div>
      </div>
