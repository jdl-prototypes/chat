  ...
  <body ng-controller='LoginController'>
    ...
    <header class='childFloatL childPad7half'>
      <div>
        ...
        <path d="M24 3c13.255 0 24 8.73 24 19.5s-10.745 19.5-24 19.5c-1.273 0-2.522-0.081-3.742-0.236-5.155 5.155-11.309 6.080-17.258 6.215v-1.262c3.213-1.574 6-4.441 6-7.717 0-0.457-0.036-0.906-0.101-1.345-5.428-3.575-8.899-9.035-8.899-15.155 0-10.77 10.745-19.5 24-19.5z" fill="#999999"></path>
        </svg>
        <span></span>
      </div>
      <div ng-click='toggleNotification()' ng-style=''>
        <svg width="30" height="30" viewBox="0 0 48 48">
        <path d="M48.070 37.5c0-13.5-12-10.5-12-21 0-0.87-0.084-1.613-0.237-2.247-0.789-5.298-4.32-9.549-8.896-10.845 0.041-0.185 0.062-0.377 0.062-0.572 0-1.56-1.35-2.836-3-2.836s-3 1.276-3 2.836c0 0.196 0.021 0.387 0.062 0.572-5.131 1.453-8.949 6.623-9.059 12.818-0.002 0.090-0.003 0.181-0.003 0.274 0 10.5-12 7.5-12 21 0 3.573 7.996 6.562 18.702 7.316 1.009 1.894 3.003 3.184 5.298 3.184s4.289-1.29 5.298-3.184c10.705-0.754 18.702-3.743 18.702-7.316 0-0.011-0.001-0.021-0.001-0.032l0.072 0.032zM38.73 40.036c-2.542 0.678-5.539 1.166-8.755 1.437-0.267-3.066-2.84-5.472-5.976-5.472s-5.708 2.406-5.976 5.472c-3.216-0.271-6.213-0.758-8.755-1.437-3.498-0.933-5.171-1.978-5.783-2.536 0.612-0.558 2.285-1.602 5.783-2.536 4.067-1.085 9.299-1.683 14.73-1.683s10.663 0.598 14.73 1.683c3.498 0.933 5.171 1.978 5.783 2.536-0.612 0.558-2.285 1.602-5.783 2.536z" fill="#999999"></path>
        </svg>
        <span></span>
        <div id='wrapperNotification' ng-show='showNotificationModal'></div>
      </div>
    </header>
    <nav>
      <div id=''>
      ...
      <div id=''>
        <li ng-repeat="user in usersList">{{user.firstName}}</li>
      </div>
    </nav>
      ...
      <div>
        <div class='defaultModalWidth'>
          ...
          <div class='pad10' ng-if='isRegisteredAlready()==false'>
            <div>
              <div class='floatL pad10' id='loginLeftPanel'>
                ...
                <input type='text' class='form-control' name='' ng-model='firstName' ng-change='change(firstName, lastName)'value=''/>
                <br/>
                <label for="exampleInputEmail1">Last Name</label>
                <input type='text' class='form-control' name='' ng-model='lastName' ng-change='change(firstName, lastName)' value=''/>
          ...
          <div class='pad10' ng-if='isRegisteredAlready()==true'>
            <label class='font30 borderBottomCCC widthFull'>Login As:</label>
    ...
    <footer>{{notificationCounter}}</footer>
  </body>
</html>